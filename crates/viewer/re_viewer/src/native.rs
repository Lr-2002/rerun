...
pub fn run_native_app(
    ...
    if crate::docker_detection::is_docker() {
        re_log::warn_once!(
            "It looks like you are running the Delta Viewer inside a Docker container. This is not officially supported, and may lead to performance issues and bugs. See https://github.com/rerun-io/rerun/issues/6835 for more.",
        );
    }

    let native_options = eframe_options(force_wgpu_backend);

    let window_title = "Delta Viewer";
    eframe::run_native(
        window_title,
        native_options,
        Box::new(move |cc| {
